...
class ExperimentResourcesInterface
  ...
  def get_workers_records(params = {})
    query = {experiment_id: @experiment_id, user_id: @user_id}
    query.merge params[:cond] if params.has_key? :cond
    opts = {}
    opts.merge params[:opts] if params.has_key? :opts
    facades = params[:infrastructure_names] || get_available_infrastructures
    facades.map {|name| [name, get_facade_for(name).sm_record_class.where(query, opts).to_a]}
  ...
  def yield_workers(records)
    records.each {|record| get_facade_for(record.infrastructure)
                               .yield_simulation_manager(record) {|worker| yield worker} }
  end
