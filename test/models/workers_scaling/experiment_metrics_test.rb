...
class ExperimentMetricsTest < ActiveSupport::TestCase
  ...
  test 'count_simulations_to_run should always return non-negative number' do
    ...
    # when
    simulations_to_run = experiment_metrics.send(:count_simulations_to_run)
    # then
  ...
  test 'count_simulations_to_run should always return current data' do
    ...
    experiment_metrics = WorkersScaling::ExperimentMetrics.new(experiment, mock)
    # when, then
    experiment_metrics.send(:count_simulations_to_run)
  end
  ...
  test 'makespan should be zero when there are no simulations to run' do
    # given
    @experiment_metrics.stubs(:count_simulations_to_run).returns(0)
  ...
  test 'makespan should be infinity when system_throughput is zero and there are simulations to run' do
    # given
    @experiment_metrics.stubs(:count_simulations_to_run).returns(1)
    @experiment_metrics.stubs(:system_throughput).returns(0)
    # when
    makespan = @experiment_metrics.makespan
    # then
  ...
  test 'makespan should return current makespan' do
    ...
    @experiment_metrics.stubs(:count_simulations_to_run).returns(simulations_to_run)
