...
require_relative '../test/db_helper'
require 'mocha'
...
class CloudClientsTest < MiniTest::Test
  ...
  def teardown
    # DO NOT DROP COLECTIONS FROM CLOUDS DB
  end
  ...
  def self.create_test_for_cloud(cloud_name, cloud_image, instance_type, params={})
    ...
    define_method "test_#{cloud_name}" do
      ...
      refute_nil secrets, "no secrets found for #{cloud_name} in database, please create"

      # client class validation
      client_class = CloudFacadeFactory.instance.client_class(cloud_name)
      refute_nil client_class
      ...
      refute_nil before_instantiate_vms
